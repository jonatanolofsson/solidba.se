 ...
 * 
 * Modified by Jonatan Olofsson
 * Copyright (c) 2009 Jonatan Olofsson
 * @name     timePicker
 * @version  0.3
...
(function($){
  ...
  $._timePicker = function(elm, settings) {
    ...
    while(time <= endTime) {
      times[times.length] = formatTime(time, settings, times[times.length-1]);
  ...
  function formatTime(time, settings,pre) {
    var h = time.getHours();
    var phrs = 0;
    if(pre != undefined) {
	    var array = pre.split(settings.separator);
	    phrs = parseFloat(array[0]);
    }
    
    
    if(phrs>0 && h==0 && settings.show24Hours) {
    	var hours = 24;
    }
    else {
